...
public class SortFileInLessMemory {
	...
	public static void main(String[] args) throws Exception {
		...
		BufferedReader br = new BufferedReader(new FileReader(new File(
				"import-meter-data.sql")));
		String s = br.readLine();
		while (s != null) {
			set.add(s);

			if (set.size() == 600) {
				FileWriter fw = new FileWriter(new File("E:/Movies/temp-" + i
						+ ".sql"));
				for (String x : set) {
					fw.write(x);
					fw.write("\n");
		...
		FileWriter fw = new FileWriter(new File("E:/Movies/temp-" + i + ".sql"));
		for (String x : set) {
			fw.write(x);
			fw.write('\n');
		}
		...
		for (int j = 0; j < brArr.length; j++) {
			brArr[j].close();
			new File("E:/Movies/temp-" + j + ".sql").delete();
		}
