...
public class AtomicStampedReferenceDemo {
	...
	static AtomicStampedReference<Person> s = new AtomicStampedReference<Person>(
			new Person(20), stampVal);
		...
		Thread t1 = new Thread(new Runnable() {
			...
			public void run() {
				...
				for (int i = 1; i <= 3; i++) {
					...
					System.out.println("stamp value for first thread:"
							+ stampVal);

					s.compareAndSet(s.getReference(),
							new Person(s.getReference().age + 10), stampVal,
							++stampVal);
		...
		Thread t2 = new Thread(new Runnable() {
			...
			public void run() {
				...
				for (int i = 1; i <= 3; i++) {
					...
					System.out.println("stamp value for second thread:"
							+ stampVal);
